...
- エージェントレビューはツール呼び出しの回数だけ伸びます。実測が近づいたら
  ...
  Cloud Tasks の HTTP ターゲットの上限 30 分を超えると起動時に落ちます）。
- **三段の数字はアプリに既定値を置きません。** 出どころはインフラ管理リポジトリ 1 箇所で、
  アプリが既定を持つと同じ数字が 2 箇所に現れ、設定漏れが「誰も選んでいない値」で
  動いてしまいます。アプリが持つ 30 分は既定値ではなく **Cloud Tasks の上限**
  （`MaxTaskDispatchDeadline`）で、これはプラットフォームの制約です。
